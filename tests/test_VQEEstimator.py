...
class TestVQEEstimatorCalibration(unittest.TestCase):
    ...
    def test_repr(self):
        self.assertEqual(repr(self.estimator_cal), "EstimatorCalibration(est_opt={}, noise_model_str=None, coupling_map_str=None, est_prim_str=aer, backend_str=aer_automatic)".format(self.est_opt))

    def test_to_dict(self):
        self.assertEqual(self.estimator_cal.to_dict(), {"name": "EstimatorCalibration", "noise_model_str": "None", "coupling_map_str": "None", "backend_str": "aer_automatic", "estimator_str": "aer", "estimator_options": self.est_opt})

    def test_get_filevector(self):
        header, data = self.estimator_cal.get_filevector()

        self.assertEqual(header, ["estimator_str", "err_mitigation", "circ_optimization", "meas_shots", "backend_str", "noise_model", "coupling_map"])
        self.assertEqual(data, ["aer", 0, 3, 1024, "aer_automatic", "None", "None"])
    def test_validate_estimator_options_aer(self):
        est_prim_str = "aer"
        est_opt = {"transpilation_options": {"optimization_level": 3}, "backend_options": {"method": "automatic", "shots": 4000}, "approximation": False, "skip_transpilation": False, "run_options": {"shots": 1024}}
        ...
        est_opt = {"transpilation_options": None, "backend_options": None, "approximation": False, "skip_transpilation": False, "run_options": None}
        ...
        est_opt = {"transpilation_options": {"optimization_level": None}, "backend_options": {"method": "automatic", "shots": 4000}, "approximation": False, "skip_transpilation": False, "run_options": {"shots": 1024}}
        val_est_opt = self.estimator_cal._validate_estimator_options(est_opt, est_prim_str)
        self.assertEqual(val_est_opt["transpilation_options"]["optimization_level"], 0)
        ...
        est_opt = {"transpilation_options": {"optimization_level": None}, "backend_options": {"method": "automatic", "shots": None}, "approximation": False, "skip_transpilation": False, "run_options": {}}

        val_est_opt = self.estimator_cal._validate_estimator_options(est_opt, est_prim_str)
        self.assertEqual(val_est_opt["backend_options"]["shots"], 1024)

        est_opt = {"transpilation_options": {"optimization_level": None}, "backend_options": {"method": "automatic"}, "approximation": False, "skip_transpilation": False, "run_options": {"shots": None}}

        val_est_opt = self.estimator_cal._validate_estimator_options(est_opt, est_prim_str)
        self.assertEqual(val_est_opt["run_options"]["shots"], 1024)

        est_opt = {"transpilation_options": {"optimization_level": None}, "backend_options": {"method": "automatic"}, "approximation": False, "skip_transpilation": False, "run_options": {}}
    ...
    def test_validate_estimator_options_ion_trap(self):
        ...
        self.assertRaises(NotImplementedError, self.estimator_cal._validate_estimator_options, est_opt, est_prim_str)
...
class TestVQEEstimator(unittest.TestCase):
    ...
    def test_parameters_setter(self):
        new_est_opt = {"transpilation_options": {"optimization_level": 0}, "backend_options": {"method": "automatic", "shots": 4000}, "run_options": {}, "approximation": False, "skip_transpilation": False}
        est_cal = VQEE.EstimatorCalibration(new_est_opt, "None", "None", "aer", "aer_automatic")
    ...
    def test_get_estimator_terra(self):
        ...
        est_cal = VQEE.EstimatorCalibration(est_opt, "None", "None", "terra", "statevector")
